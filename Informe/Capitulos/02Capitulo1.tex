...
\begin{enumerate}
	...
	\item \textbf{Vista:} es la responsable de presentar las vistas al usuario e interactuar con el mismo.
...
\begin{figure}[t]
	\begin{center}
		...
		\caption[Patrón de diseño Modelo-Vista-Controlador.]{Patrón de diseño Modelo-Vista-Controlador. Recuperado de \cite{mvp2017guide}}
...
Una vez que fue recolectada toda la información que se describió durante la sección anterior, se procede a medir el grado de similitud que existe entre cada par de imágenes, basándose en las etiquetas y probabilidades que se obtuvo de cada una de ellas, como así también del vector de características provisto por la capa de \textit{embeddings}. La idea es en el primer caso aprovechar la información relacionada a los ``objetos'' (i.e. animales, vasos, árboles, etc), personas o lo que sea que se encuentre en la imagen desde un punto de vista gramatical, considerando clases de objetos que puedan estar relacionados. Y en segundo lugar, a través de los \textit{embeddings}, lo que se busca es usar la información de más bajo nivel, encontrando similitudes que puedan tener las imágenes en cuanto a magnitudes físicas entre sí.

Al final de esta etapa, se obtendrá una matriz que será utilizada por el algoritmo de \textit{clustering}. Dicha matriz, es el resultado de una suma ponderada de dos matrices que reciben el nombre de \emph{matrices de afinidad}, las cuales se describen a continuación:
...
\begin{itemize}
	\item \textbf{Potencia de expansión:} indica cuánto deben explorarse los grafos obtenidos a partir de la matriz de afinidad. El valor a elegir se recomienda que varíe entre 2 y 3\footnote{\url{http://dogdogfish.com/mathematics/markov-clustering-what-is-it-and-why-use-it/}}.
	
	\item \textbf{Potencia de inflación:} A medida que se aumenta este valor, crece la cantidad de grupos que se obtendrán. De manera similar al concepto anterior, el valor a utilizar se recomienda que varíe entre 2 y 4.
	
	\item \textbf{Umbral de corte:} diferencia máxima que debe existir entre la suma de los valores absolutos de los elementos de la matriz actual contra la suma de los valores absolutos de los elementos de la matriz en la iteración anterior.
