...
En este capítulo se presenta al sistema operativo Android, plataforma en la que se realizó el desarrollo y despliegue de la aplicación. También, se exponen los fundamentos del aprendizaje profundo referido al enfoque utilizado en este proyecto respecto a visión computacional. Por último, se introducen contenidos acerca de algoritmos de clustering, en especial conceptos que abarcan las características del algoritmo utilizado en este trabajo.
...
Uno de los ámbitos multimedia con más crecimiento en los últimos años ha sido el de los dispositivos móviles. La llegada de los \textit{smartphones} disparó en su momento la creación de aplicaciones móviles que aprovechan la capacidad multimedia de estos dispositivos. La aparición y el despegue poco después de las tabletas ha convertido el desarrollo de aplicaciones para dispositivos móviles en un pilar de la industria multimedia.

Existen lenguajes o \textit{frameworks} que nos permiten la creación de programas multidispositivo (se programa una vez y se ejecuta en cualquier dispositivo), pero para aplicaciones complejas que requieren un gran rendimiento, la programación suele hacerse en los lenguajes nativos de cada sistema operativo, con lo que debe reescribirse la aplicación para cada uno de ellos \cite{programacionMobile}. Como se ha dicho anteriormente, en este trabajo el desarrollo de la aplicación se hizo sobre el lenguaje nativo del sistema operativo Android.
%------------------------------------------------------------------
\subsection{Plataforma Android}
%-------------------------------------------------------------------
\label{cap2:subsec:plataforma-android}
Es un sistema operativo diseñado para dispositivos móviles, desarrollado inicialmente por \textit{Android Inc.}, una empresa comprada por Google en 2005. El lanzamiento se realizó el 5 de noviembre de 2007 y Google liberó la mayoría de código Android bajo licencia Apache, conocida por ser libre y de código abierto  \cite{dimarzio2008programmer}.

Actualmente, se pueden encontrar más de 2.500.000 aplicaciones desarrolladas para esta plataforma \cite{numberAndroidApps}. Además, cuenta con un entorno de desarrollo integrado oficial (IDE), denominado \textit{Android Studio}, realizado por la empresa JetBrain. El mismo fue anunciado en mayo de 2013 y reemplazó a Eclipse como IDE oficial para el desarrollo de aplicaciones \cite{androidStudioOficial}.
...
\begin{itemize}
	...
	\item Las aplicaciones se proveen a través de los \textit{Android Application Package} (APK), similar a los archivos con extensión .exe en Windows. Las mismas deben ser firmadas digitalmente antes de poder ser desplegadas. Una opción totalmente válida es firmar la aplicación en modo depuración, en contraposición a otros sistemas operativos que cuentan con este tipo de restricciones, permitiendo agilizar en términos de desarrollo y testing \cite{appSigning}.
...
\begin{itemize}
	\item \textbf{Activity:} componente principal de la interfaz gráfica de una aplicación. A cada Activity se le asigna una ventana en la cual se dibuja la interfaz de usuario, donde éste podrá interaccionar para realizar las diversas acciones que hayamos contemplado en la aplicación. 
	
	Para construir la interfaz gráfica, existen componentes denominados \textit{Views} (vistas) con los que se dispone de numerosos controles básicos, como por ejemplo, botones, listas desplegables o cuadros de texto, pudiendo extender la funcionalidad de estos controles o crear otros personalizados.
	...
	\item \textbf{Content Provider:} es un objeto destinado a compartir datos entre aplicaciones. Dichos datos pueden ser almacenados en el sistema de archivos, en una base de datos o en cualquier otro lugar que sea accesible desde la aplicación. Un ejemplo de Content Provider es el que utiliza Android para gestionar la información de un contacto, mediante el cual cualquier otra aplicación podrá acceder a ellos haciendo una petición sobre la interfaz que gestiona dicha herramienta (\textit{ContentResolver})
...
Cuando una actividad entra y sale de los diferentes estados que se describieron arriba, se notifica a través de diferentes métodos \textit{callback}. En conjunto, estos métodos definen el ciclo de vida completo de una actividad, como se puede observar en la Figura \ref{fig:activity-lifecycle}.

\begin{figure}[t]
	\begin{center}
		\includegraphics[width=0.8\textwidth]%
		{Imagenes/Bitmap/activity_lifecycle}
		\caption[Ciclo de vida de una actividad.] {Ciclo de vida de una actividad. Recuperado de: \url{https://developer.android.com/guide/components/activities}}
...
%-------------------------------------------------------------------
\section{Inteligencia Artificial, Aprendizaje Maquinal y Aprendizaje Profundo}
%-------------------------------------------------------------------
...
En esta sección, se realiza una introducción sobre conceptos relacionados a redes neuronales, aprendizaje maquinal y aprendizaje profundo, criterios que pertenecen a la inteligencia artificial. El último de ellos, en particular, fue fundamental para la caracterización de las imágenes en este proyecto.
...
Para comenzar, se debe definir claramente de que se está hablando cuando se menciona a la inteligencia artificial. ¿De qué se tratan los conceptos de inteligencia artificial (\textbf{AI}, del inglés \textit{``Artificial Intelligence''}), aprendizaje maquinal y aprendizaje profundo (\textbf{DL}, del inglés \textit{``Deep Learning''})? ¿Cómo están relacionados entre ellos? (ver Figura \ref{fig:ai-ml-dl})
...
La inteligencia artificial nació en los años 50s, cuando un conjunto de pioneros dedicados al campo de las ciencias de la computación se comenzaron a preguntar cómo las computadoras podrían ser hechas para ``pensar''. Una concisa definición del campo podría ser la que sigue: \textit{el esfuerzo de automatizar tareas intelectuales, normalmente realizadas por humanos} \cite{chollet2017deep}. Como tal, la AI es un campo general que incluye al aprendizaje maquinal y al profundo, pero que también incluye muchos más conceptos que no involucran ningún \textit{aprendizaje}. Los primeros juegos de ajedrez por computadora, por ejemplo, sólo envolvían muchísimas reglas preestablecidas escritas por programadores con un alto nivel de conocimiento y de ayuda proveniente de expertos en la materia. Este concepto es conocido como ``inteligencia artificial simbólica'', y fue muy popular entre los años 50 y 80 gracias al boom de los sistemas expertos creados en esos años \cite{chollet2017deep}.

Aunque la AI simbólica provee buenas resoluciones para problemas bien definidos, lógicos, tales como el ajedrez, estos no sirven para obtener buenas aproximaciones a problemas relacionados a la clasificación de imágenes, reconocimiento del habla y traducción de lenguaje, entre otros. Por lo que una nueva aproximación tomó el lugar de la AI simbólica: el aprendizaje maquinal.
...
El aprendizaje maquinal surge a partir de esta pregunta: ¿puede una computadora ir mas allá ``de lo que los humanos saben'' con el objetivo de conseguir mejores resultados y aprender por ella misma cómo mejorar tareas concretas, sin que se le especifique cómo hacerlo? \cite{chollet2017deep}.

Esta pregunta abrió la puerta a un nuevo paradigma de programación. En la programación clásica, como por ejemplo la inteligencia artificial simbólica, el humano escribe todas las reglas (a través de sus programas) y los datos se procesan acorde a lo que ellas dicen, y luego se obtiene una posible respuesta (ver Figura \ref{fig:machinelearningnewparadigm}). En cambio, con el ML, tanto los datos de entrada como la respuesta esperada son conocidos por parte de los humanos, entonces lo que realmente importa son las reglas que el sistema ``aprende'' automáticamente, ya que estas serán aplicadas a nuevos datos de entrada para producir respuestas originales \cite{chollet2017deep}.
...
Un sistema realizado bajo el paradigma del aprendizaje maquinal es \textit{entrenado} más que explícitamente programado. Es decir, se le presentan muchos ejemplos relevantes a una tarea, y él por sus propios medios encontrará la estructura más acorde para obtener la respuesta esperada. Por lo tanto, una vez que ya se encuentra bien entrenado, permitirá resolver tareas de ese tipo de manera automática, siendo capaz de generalizar a partir de esos ejemplos.
...
La tarea descripta arriba es un claro ejemplo de un problema de \textit{clasificación}, altamente relacionado a la problemática tratada en este proyecto donde el ML encuadra perfectamente como opción de resolución. Otros ejemplos de aplicación podrían ser problemas de regresión, \textit{clustering}, etc. \cite{alpaydin2009introduction}.
...
Las \textit{Redes Neuronales Artificiales} (\textbf{ANN}, del inglés \textit{Artificial Neural Netwroks}) comprenden una rama antigua pero destacada, especialmente en la actualidad debido al surgimiento del aprendizaje profundo y a los avances en lo que refiere a hardware de procesamiento (GPU). Se entiende como ANN a un sistema con elementos (neuronas) que procesan información de cuyas interacciones locales depende su comportamiento global \cite{gonzalez1995redes}. Este sistema está inspirado en el comportamiento del cerebro humano, adquiriendo conocimiento de su entorno mediante un proceso de aprendizaje y almacenándolo para disponer de su uso \cite{haykin2004comprehensive}.

Las ANNs son implementadas en computadoras, tanto en programas de software como en arquitecturas de hardware, para componer sistemas de aprendizaje maquinal. Para modelar el comportamiento de las neuronas, la idea es que las sinápsis que producen pueda controlarse mediante \textit{pesos sinápticos} que definan una magnitud de la influencia que ejerce una con otra (y también dirección, al poder excitar o inhibir mediante pesos positivos o negativos, correspondientemente). La ecuación que rige al modelo es $\sum_{i} w_i x_i + b$, donde los pesos sinápticos $w_i$ multiplican las señales de entrada $x_i$ para ejercer una suma ponderada en el soma. Si el resultado supera un cierto umbral la neurona se ``activa'' enviando un estímulo hacia su salida. Esta suma puede incorporar además un término de sesgo $b$ que participa sin multiplicarse por la entrada \cite{learninspy2016tesis}. En este modelo se considera que no interesa conocer el preciso momento en que se activa la neurona sino la frecuencia en que ocurre, representado mediante una \textit{función de activación} \cite{haykin2004comprehensive}. Dicha función determina cómo se transforman las entradas a través de una red neuronal, lo cual es determinante para que logre su capacidad de aprender funciones complejas. Originalmente las funciones de activación más utilizadas eran las sigmoideas, las cuales son la \textit{sigmoidea} ($\sigma$)  y la \textit{tangente hiperbólica} ($tanh$). Las mismas tienen una inspiración biológica y están delimitadas por un mínimo y un máximo valor, lo cual causa que las neuronas se saturen en las últimas capas de la red neuronal \cite{van2014analysis}. Otra función de activación que ha sido muy utilizada en muchas aplicaciones es la \textit{lineal rectificada} o ReLU, la cual comprende una no linealidad simple: resulta en 0 para entradas negativas, y para valores positivos se mantiene intacta, por lo cual no tiene valores límites como las funciones sigmoideas. El gradiente de la ReLU es 1 para todos los valores positivos y 0 para los negativos, lo cual hace que, durante la optimización de la red, los gradientes negativos no sean usados para actualizar los pesos sinápticos correspondientes. Además, el hecho de que el gradiente sea 1 para cualquier valor positivo hace que el entrenamiento sea más rápido que con otras funciones de activación no lineales \cite{learninspy2016tesis}. En la Figura \ref{fig:neurona_modelo} se representa gráficamente el modelo explicado, el cual constituye lo que se denomina como ``perceptrón simple'. 

\begin{figure}[t]
	\begin{center}
		\includegraphics[width=0.7\textwidth]%
		{Imagenes/Bitmap/neuron_model}
		\caption [Modelo matemático de una neurona.] {Modelo matemático de una neurona. Recuperado de \cite{learninspy2016tesis}}
		\label{fig:neurona_modelo}
	\end{center}
\end{figure}

Para modelar una red neuronal artificial las neuronas se agrupan en capas, de forma tal que todas las unidades de una capa se conectan con todas las neuronas de sus capas próximas para formar una estructura interconectada. En la forma más simple, se tiene una capa de entrada que proyecta la señal entrante en una capa de salida. Cuando se incorporan capas intermedias (denominadas capas ocultas), la red adquiere más niveles de procesamiento entre su entrada y salida, y lo que se forma es un ``perceptrón multicapa'' (\textbf{MLP}, del inglés \textit{Multi Layer Perceptron}) \cite{haykin2004comprehensive}. En esta configuración, para producir la salida de la red se computan sucesivamente todas las activaciones una capa tras otra, donde la primer salida es la entrada de la segunda y así sucesivamente. Además, cada capa puede tener diferente número de neuronas, e incluso distinta función de activación. 

\begin{figure}[t]
	\begin{center}
		\includegraphics[width=0.85\textwidth]%
		{Imagenes/Bitmap/arq-mlp}
		\caption[Arquitectura básica de un MLP con 4 capas.]{Arquitectura básica de un MLP con 4 capas. Recuperado de \cite{learninspy2016tesis}}
		\label{fig:mlp}
	\end{center}
\end{figure}

Siguiendo el modelo matemático de un perceptrón simple, los pesos sinápticos ahora pueden expresarse en conjunto de forma vectorial como matrices, así como también el sesgo se representa como un vector. Por lo tanto, dada un vector de entrada \textbf{x}, la suma ponderada efectuada en una capa se expresa como un producto entre la matriz de pesos sinápticos $W$ y dicha entrada \textbf{x}, en la cual también se suma el vector de sesgo o \textit{bias} \textit{\textbf{b}}. Al resultado de esto se le aplica la función de activación, con lo cual se produce la salida final de la capa. En la Figura \ref{fig:mlp} se representa la arquitectura de un perceptrón multicapa, mostrando las interacciones que tienen sus unidades desde la entrada hasta su salida \cite{learninspy2016tesis}. 

\subsubsection{Aprendizaje Supervisado}

En el aprendizaje supervisado, los datos de entrenamiento consisten de pares de objetos: una componente del par son los datos de entrada y el otro, los resultados deseados. La salida de la función puede ser un valor numérico (como en los problemas de regresión) o una etiqueta de clase (como en los de clasificación). El objetivo del aprendizaje supervisado es el de crear una función capaz de predecir el valor correspondiente a cualquier objeto de entrada válido después de haber visto una serie de ejemplos: los datos de entrenamiento. Para ello, tiene que generalizar a partir de los datos presentados a las situaciones no vistas previamente \cite{izenman2008modern}.

El fin de entrenar una red neuronal mediante aprendizaje supervisado es el de minimizar el error entre la salida deseada, y la salida real. Para ello se ajustan los pesos $w_i$ de las conexiones de la red en función de la diferencia entre los valores deseados y los obtenidos a la salida, es decir, en función del error cometido en la salida. Un algoritmo muy utilizado es el de propagación hacia atrás o \textit{backpropagation}. El método emplea un ciclo propagación - adaptación de dos fases: una vez que se ha aplicado un ejemplo a la entrada de la red, este se propaga desde la primera capa a través de las capas siguientes, hasta generar una salida. La señal de salida se compara con la salida deseada y se calcula una señal de error. Las salidas del error se propagan hacia atrás, partiendo desde la capa de salida, hacia todas las neuronas de la capa oculta que contribuyen directamente al resultado \cite{rojas2013neural}. Sin embargo, las neuronas de la capa oculta solo reciben una fracción de la señal total del error, basándose aproximadamente en la contribución relativa que haya aportado cada neurona a la salida original. Este proceso se repite, capa por capa, hasta que todas las neuronas de la red hayan recibido una señal de error que describa su contribución relativa al error total. 

Existen diferentes funciones que se utilizan para minimizar el error en las neuronas, denominadas funciones de ``costo'' o ``penalización''. El objetivo es ir reduciendo, iteración a iteración durante el entrenamiento, dicho error hasta que se estabilice. El método más comúnmente utilizado para encontrar el punto mínimo es el del \textit{gradiente descendiente} \cite{bishop2006pattern}. Se debe pensar a la función como una montaña ondulada y el descenso por gradiente es como deslizarse por la montaña para alcanzar el punto más bajo. No hay una sola función de ``costo'' que funcione para todo tipo de datos, ya que depende de una serie de factores como la problemática a tratar, la elección del algoritmo de ML, la eficiencia en el tiempo del descenso por gradiente y la confianza en las predicciones. Algunos de los métodos más utilizados son: Error Cuadrático Medio, \textit{cross-entropy}, \textit{Hinge}, entre otros \cite{LossFunction}. 

La importancia de este proceso consiste en que, a medida que se entrena la red, las neuronas de las capas intermedias se organicen a sí mismas de tal modo que aprenden a reconocer distintas características del espacio total de entrada. La meta es conseguir un modelo que pueda \textbf{generalizar} a partir de los datos con los que fue entrenado, con el objetivo de no sobre-entrenarlo, y que sepa de ``memoria'' detalles de los datos de entrenamiento. Este fenómeno es conocido como \textit{overfitting}, donde el modelo reconoce toda la información relevante de los datos de entrenamiento, pero falla considerablemente cuando se le presentan nuevos ejemplos.

Lo principal es que después del entrenamiento, cuando se les presente un patrón arbitrario de entrada que contenga ruido o que esté incompleto, las neuronas de la capa oculta de la red respondan con una salida activa si la nueva entrada contiene características que se asemeje a aquellas que las neuronas hayan aprendido a reconocer \cite{rojas2013neural}. 
...
El \textit{deep learning} es un subcampo específico del aprendizaje maquinal: una nueva manera de aprender desde los datos poniendo énfasis en sucesivas \textit{capas} desde las cuales se van obteniendo representaciones cada vez más significativas. Lo profundo no hace referencia a ningún tipo de entendimiento profundo logrado por este enfoque, sino a la idea de sucesivas capas de representación por las que van pasando los datos. Modelos actuales envuelven entre diez o incluso miles de capas sucesivas de representación y todas ellas aprenden de manera automática a partir de los datos de entrada.

Estas capas de representación están conformadas por modelos llamados \textit{redes neuronales}. Como se dijo anteriormente, este término está referenciado en la neurobiología, pero aunque algunos de los conceptos centrales de las redes neuronales fueran desarrollados en parte tomando como inspiración el entendimiento humano del cerebro, los modelos \textit{deep learning} no son modelos del cerebro. No existe evidencia de que el cerebro humano implemente algo parecido a lo que se hace en esta ciencia de la computación \cite{chollet2017deep}.
...
A partir de la introducción sobre CV realizada en la Sección \ref{cap1:sec:motivation}, así como los antecedentes de sus aplicaciones exitosas mencionados allí también, se profundiza acerca de las diferencias que ofrece la resolución de problemas de visión computacional aplicando técnicas de aprendizaje profundo, comparando contra las metodologías convencionales y utilizadas para el procesamiento digital de imágenes.

En las metodologías convencionales, el punto de atención sobre la manera en que se realiza la \textbf{extracción de características}, está en el campo del procesamiento de señales y procesamiento digital de imágenes, donde los algoritmos han sido diseñados para adquirir y extraer características de ellas, tal como contornos, bordes, texturas, etc. Existen una gran variedad de técnicas y algoritmos, tanto libres como con licencia. Por ejemplo: SIFT y SURF los cuales se encuentran bajo patente, y ORB, FAST y BRISK los cuales son libres \cite{karami2017image}. Todos ellos, describen de alguna manera a las imágenes y son utilizados en problemas relacionados a la identificación de objetos.

Por otro lado, los algoritmos de visión computacional que aplican aprendizaje profundo, transforman las imágenes digitales en representaciones que son diferentes a la imagen original. Se puede pensar a una red profunda como una operación de muchas etapas donde la información se va destilando, ya que los datos pasan a través de sucesivos filtros y sale \textit{purificada} (ver Figura \ref{fig:deepRepresentation}) \cite{chollet2017deep}.

\begin{figure}[t]
	\begin{center}
		\includegraphics[width=1\textwidth]%
		{Imagenes/Bitmap/deepRepresentation}
		\caption[Representación de un modelo de aprendizaje profundo, para clasificación de dígitos manuscritos.]{Representación de un modelo de aprendizaje profundo, para clasificación de dígitos manuscritos. Recuperado de \cite{chollet2017deep}}
		\label{fig:deepRepresentation}
	\end{center}
\end{figure}

Estas redes se denominan convolucionales (\textbf{CNN}, del inglés \textit{Convolutional Neural Network}) y actualmente son el estado del arte en lo que refiere a problemas de clasificación de imágenes \cite{introCNN}. Las CNN aplican una serie de filtros entrenados a partir de los datos sobre la información cruda de las imágenes (píxeles) para extraer y aprender características, las cuales serán utilizadas por el modelo para realizar la clasificación. Estas redes están compuestas por tres componentes:
...
\begin{itemize}
	...
	\item \textbf{Capas pooling:} obtienen los datos extraídos por las capas convolucionales con el objetivo de reducir la dimensión en orden de acortar los tiempos de proceso y quedarse con las características más comunes. Esto se lleva a cabo a partir de operaciones sobre los datos como promedio, máximo, mínimo, etc.
...
Es una biblioteca de software libre que se utiliza para realizar cálculos numéricos mediante diagramas de flujo de datos. Fue desarrollada por Google para satisfacer sus necesidades de sistemas capaces de construir y entrenar redes neuronales para detectar y descifrar patrones y correlaciones, análogos al aprendizaje y razonamiento usados por los humanos. Actualmente es utilizado tanto en la investigación como en los productos de Google. Fue originalmente desarrollado por el equipo de \textit{Google Brain} para uso interno de la compañía antes de ser publicado bajo la licencia de código abierto Apache 2.0, el 9 de noviembre de 2015 \cite{abadi2016tensorflow}. TensorFlow es multiplataforma, funciona tanto en GPUs como CPUs -incluyendo plataformas móviles y embebidas- e incluso unidades de procesamiento tensorial (TPUs), que son un hardware especializado en realizar cálculos tensoriales. 
...
Son una familia de modelos de redes neuronales profundas utilizadas en problemas de visión computacional, diseñados para maximizar la precisión de forma efectiva, teniendo en cuenta los recursos restringidos para una aplicación integrada en un dispositivo móvil. Son modelos pequeños, de baja latencia y baja potencia, parametrizados para satisfacer las limitaciones de recursos de una gran variedad de casos de uso. Se pueden utilizar para la clasificación, detección, incrustación y segmentación de imágenes \cite{howard2017mobilenets}. Las MobileNet son similares a los modelos Inception V3 (un tipo de CNN desarrolladas por Google, utilizadas para resolver problemas de clasificación de imágenes bajo un concepto denominado ``transferencia de aprendizaje'' logrando muy buenas métricas \cite{xia2017inception}), pero más pequeñas y con un desempeño un poco más bajo \cite{MobileNetInceptionV3}. Estos modelos pueden ser descargados y ajustados a proyectos de manera libre y gratuita, y existen diferentes tipos, variando sobre todo en la optimización que se necesite para cada caso de aplicación. En el siguiente capítulo se ahondará sobre el uso particular que tiene en este proyecto final de carrera. 
...
El análisis de agrupamiento (\textit{clustering}) es la tarea de agrupar un conjunto de objetos de tal manera que los miembros del mismo grupo (llamado cluster) sean más similares, en algún sentido o bajo algún criterio, y a la vez menos similares a elementos de otros clusters. Es la tarea principal de la minería de datos y es una técnica común en el análisis de datos estadísticos. Es utilizada también en múltiples campos como el aprendizaje automático, la bioinformática, la compresión de datos y la computación gráfica \cite{kogan2007introduction}.

El análisis de grupos no consiste en un algoritmo específico. Existen diferentes enfoques de \textit{clustering} que difieren significativamente en su idea de qué constituye un grupo y cómo encontrarlos eficientemente. Las ideas clásicas de grupo incluyen distancias mínimas entre los miembros del mismo, áreas densas del espacio de datos, intervalos o distribuciones estadísticas particulares. El algoritmo apropiado y los valores de los parámetros (incluyendo valores como la función de distancia para utilizar, un umbral de densidad o el número de grupos esperado) depende del conjunto de datos que se analiza y el uso que se le dará a los resultados. El \textit{clustering} como tal, es un proceso iterativo de minería de datos que implica prueba y fracaso, el cual se entrena de manera no supervisada, es decir, la idea es que el algoritmo descubra automáticamente cuales son las características que permiten agrupar distintos elementos. A menudo es necesario hacer un pre-procesamiento de los datos y un ajuste de los parámetros del modelo hasta que el resultado tenga las propiedades deseadas \cite{kogan2007introduction}. 

La idea de un clúster o grupo resulta compleja para concretar su definición exacta, es por ello que existen numerosos algoritmos de agrupamiento \cite{estivill2002so}.  En todos ellos hay un común denominador: un grupo de datos. Aun así, los investigadores emplean diferentes modelos de grupo, y para cada uno de estos modelos se utilizan diferentes algoritmos. La idea de un grupo, cuando es encontrado por algoritmos diferentes, varía significativamente en sus propiedades. Típicamente los modelos de grupo incluyen:

\begin{itemize}
	\item Modelos de conectividad: por ejemplo, el agrupamiento jerárquico construye modelos basados en la distancia de las conexiones.
	
	\item Modelos de centroide: por ejemplo, el algoritmo \textit{k-means} representa cada grupo por un solo vector medio \cite{wagstaff2001constrained}.
	
	\item Modelos de distribución: los grupos son modelados utilizando distribuciones estadísticas, como la distribución normal y la varianza, por ejemplo.
	
	\item Modelos basados en grafo: los grupos están formados por un subconjunto de nodos, tal que para dos nodos en el subconjunto existe una arista que los conecta siguiendo algún criterio.
\end{itemize}

\subsection{Agrupamiento basado en grafos}

Para este proyecto, el algoritmo de agrupamiento utilizado pertenece a un modelo basado en grafos (en ingles: \textit{graph clustering}) \cite{schaeffer2007graph}. Los grafos son estructuras de datos en las cuales los nodos representan entidades, y las aristas representan relaciones (ver Figura \ref{fig:graph-cluster}). Por ejemplo, una persona podría construir un grafo sobre las conexiones que existen entre sus amigos en Facebook. Allí, cada nodo se correspondería con sus amigos, y las aristas representarían una relación de amistad entre dos de ellos. 
...
Para el caso de aplicación abordado durante este proyecto, se pensó a cada imagen como una entidad y a cada arista como el grado de similitud o contexto que existe entre ellas. El algoritmo utilizado para llevar adelante esta tarea encuadra dentro de esta categoría y se denomina  ``agrupamiento de Markov'' (\textbf{MCL}, del inglés \textit{Markov Clustering}), el cual se introduce a continuación \cite{van2000graph}.

\subsubsection{Algorítmo de agrupamiento de Markov}

MCL es un algoritmo de clúster rápido y escalable para grafos (también conocido como redes) basado en la simulación del flujo (estocástico) en grafos \cite{van2000graph}. Fue desarrollado por Stijn van Dongen en el Centro de Matemáticas e Informática (CWI) en Holanda. La tesis doctoral \textit{Graph clustering by flow simulation} \cite{van2000graph} presenta este algoritmo, teniendo como temas principales la teoría matemática que lo sustenta, su posición en el análisis de clústeres, cuestiones relacionadas con la escalabilidad, la implementación, y los criterios de rendimiento para la agrupación de grafos en general. Lo que hace es simular el flujo usando dos operaciones algebraicas simples en matrices y no requiere instrucciones de procedimiento para ensamblar, unir o dividir los grupos \cite{dongen2000cluster}. 

La primer operación utilizada es la expansión, que coincide con la multiplicación normal de la matriz. La expansión modela la dispersión del flujo, haciéndola más homogénea. La segunda es la inflación, que es, matemáticamente hablando, una potencia de Hadamard\footnote{\url{https://www-fourier.ujf-grenoble.fr/~parisse/giac/doc/en/cascmd_en/node427.html}} seguida de una escala diagonal. La inflación modela la contracción del flujo, haciéndose más espesa en las regiones de mayor corriente y más delgada en las regiones de menor corriente. El proceso MCL hace que el flujo se extienda dentro de los cúmulos naturales y se evapore entre los diferentes cúmulos. En la Figura \ref{fig:mcl} se puede observar un ejemplo de 4 iteraciones de un proceso MCL.

\begin{figure}[t]
	\begin{center}
		\includegraphics[width=0.85\textwidth]%
		{Imagenes/Bitmap/mcl}
		\caption{Representación gráfica de la evolución del resultado de MCL}
		\label{fig:mcl}
	\end{center}
\end{figure}
...
El proceso por lo general converge rápido y en pocas iteraciones. Esto da los medios para escalar el algoritmo drásticamente, lo que conduce a una complejidad que en el peor de los casos es del orden $O(N k^2)$, donde N es el número de nodos del grafo de entrada, y k es un umbral para el número de recursos asignados a cada nodo \cite{van2000graph}.
