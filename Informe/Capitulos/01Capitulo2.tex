...
\begin{itemize}
	...
	\item Las aplicaciones se proveen a través de los \textit{Android Application Package} (APK), similar a los archivos con extensión .exe en Windows. Las mismas deben ser firmadas digitalmente antes de poder ser desplegadas. A pesar de ello, no poseen ninguna restricción en comparación a las que tienen otros sistemas operativos, agilizando en términos de desarrollo y testing, ya que una opción totalmente válida es firmar la aplicación en modo depuración \cite{appSigning}.
...
Esta pregunta abrió la puerta a un nuevo paradigma de programación. En la programación clásica, como por ejemplo la inteligencia artificial simbólica, el humano escribe todas las reglas (a través de sus programas) y los datos se procesan acorde a lo que ellas dicen, y luego se obtiene una posible respuesta (ver Figura \ref{fig:machinelearningnewparadigm}). En cambio, con el ML, tanto los datos de entrada como la respuesta esperada por ellos son tan bien conocidos por parte de los humanos, que realmente lo que importa son las reglas que el sistema ``aprende'' automáticamente, ya que estas serán aplicadas a nuevos datos de entrada para producir respuestas originales \cite{chollet2017deep}.
...
La importancia de este proceso consiste en que, a medida que se entrena la red, las neuronas de las capas intermedias se organicen a sí mismas de tal modo que aprenden a reconocer distintas características del espacio total de entrada. La meta es conseguir un modelo que pueda \textbf{generalizar} a partir de los datos con los que fue entrenado, con el objetivo de no sobre-entrenarlo, y que sepa de ``memoria'' detalles de los datos de entrenamiento. Este fenómeno es conocido como \textit{overfitting}, donde se ve cómo el modelo reconoce toda la información relevante de los datos de entrenamiento, pero falla considerablemente cuando se le presentan nuevos ejemplos.

Lo principal es que después del entrenamiento, cuando se les presente un patrón arbitrario de entrada que contenga ruido o que esté incompleto, las neuronas de la capa oculta de la red respondan con una salida activa si la nueva entrada contiene características que se asemeje a aquellas que las neuronas individuales hayan aprendido a reconocer \cite{rojas2013neural}. 
...
\begin{itemize}
	...
	\item Modelos basados en grafo: los grupos están formados por un subconjunto de nodos, tal que cada dos nodos en el subconjunto exista una arista que los conecta siguiendo algún criterio.
\end{itemize}
